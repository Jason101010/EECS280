...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
    int col;
    col = Matrix_column_of_min_value_in_row
    (cost, Matrix_height(cost) - 1, 0, Matrix_width(cost));
    seam[Matrix_height(cost) - 1] = col;
    for (int r = Matrix_height(cost) - 2; r >= 0; --r) {
        if (col == 0) {
            col = Matrix_column_of_min_value_in_row(cost, r, col, col + 2);
            seam[r] = col;
        }
        else if (col == Matrix_width(cost) - 1) {
            col = Matrix_column_of_min_value_in_row(cost, r, col - 1, col + 1);
            seam[r] = col;
        }
        else {
            col = Matrix_column_of_min_value_in_row(cost, r, col - 1, col + 2);
            seam[r] = col;
...
void remove_vertical_seam(Image* img, const int seam[]) {
    ...
    Image_init(removed, Image_width(img) - 1, Image_height(img));
    for (int r = 0; r < Image_height(img); ++r) {
        for (int c = 0; c < seam[r]; ++c) {
            Image_set_pixel(removed, r, c, Image_get_pixel(img, r, c));
        }
        for (int c = seam[r] + 1; c <= Image_width(removed); ++c) {
            Image_set_pixel(removed, r, c - 1, Image_get_pixel(img, r, c));
        }
    }
    *img = *removed;
    delete removed;
