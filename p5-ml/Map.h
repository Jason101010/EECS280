...
public:
  ...
  Iterator find(const Key_type& k) const {
      Iterator it = a.find({ k, Value_type() });
      return it;
  }
  ...
  Value_type& operator[](const Key_type& k) {
      Iterator it = a.find({ k, Value_type() });
      if (it == a.end()) {
          a.insert({ k, Value_type() });
          Iterator it2 = a.find({ k, Value_type() });
          return (*it2).second;
      }
      else {
          return (*it).second;
  ...
  std::pair<Iterator, bool> insert(const Pair_type& val) {
      Iterator it = a.find(val);
      if (it == a.end()) {
          a.insert(val);
          Iterator it2 = a.find(val);
...
private:
    BinarySearchTree<Pair_type, PairComp> a;
};
