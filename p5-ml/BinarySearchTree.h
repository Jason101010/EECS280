...
#include <functional> //less
 ...
 //Can we modify datum by dereferencing iterator
 //INSERT!!!!!!!!!!!!!!!!!!!!!!!!!!!
 //Where does the data-representation private belong to??
 //node == nullptr?? -->Lecture20 List length
 //copy_node_impl: not assigning recursive call to a variable
 //What are the data types of the paramters of comparator less
 //Can static function in private access other static functions?
 //How to tease out which can use which
 //Can we deference a nullptr? No
...
private:
    ...
    static Node* copy_nodes_impl(Node* node) {
        if (!node) {
            ...
            new_copy->datum = node->datum;
            new_copy->left = copy_nodes_impl(node->left);
            new_copy->right = copy_nodes_impl(node->right);
            return new_copy;
        ...
        if (!node) {
            ...
            if (!(node->left) && !(node->right)) {
                ...
                return (less(node->left->datum, node->datum)) &&
                    ...
                    (!(node->left->right) || 
                        less(node->left->right->datum, node->datum)) &&
                    (!(node->right->left) ||
                        less(node->datum, node->right->left->datum)) &&
